...
class IndexTTSManagerFlet:
    def __init__(self):
        ...
        try:
            ...
            if os.path.exists(version_file):
                with open(version_file, "r", encoding="utf-8") as f:
                    self.app_version = json.load(f).get("version", "3.4.0")
        ...
        self.log_manager = LogManager(f"IndexTTS{self.app_version}_Manager", log_level)
    ...
    def setup_page(self):
        """设置页面属性"""
        self.page.title = f"IndexTTS2 多功能启动器 v{self.app_version}"
    ...
    def setup_ui(self):
        ...
        self.page.appbar = ft.AppBar(
            title=ft.Text(f"IndexTTS2 多功能启动器 v{self.app_version}", size=20, weight=ft.FontWeight.BOLD),
            center_title=True,
...
内容完整：保留全部原文内容，不可删减，并且按照原文顺序进行分割，顺序不能打乱，内容不可缺少。
    ...
    def show_settings_dialog(self, e):
        ...
        # 科技风设置对话框 - 采用标签页分组+卡片布局
        appearance_tab = ft.Container(
            content=ft.Column(
                [
                    ft.Text("界面与体验", size=14, weight=ft.FontWeight.BOLD),
                    ft.Text("调整主题、字号、播放节奏，让界面更符合你的习惯。", size=11, color=ft.Colors.GREY_600),
                    ft.Divider(height=12),
                    ft.Row([ft.Text("主题模式", size=12, width=80), theme_dropdown], spacing=10),
                    ft.Container(
                        content=ft.Row([
                            ft.Text("界面字体", size=12, width=80),
                            font_size_slider,
                        ], spacing=10),
                    ),
                    ft.Container(
                        content=ft.Row([
                            ft.Text("音频间隔", size=12, width=80),
                            audio_interval_slider,
                        ], spacing=10),
                    ),
                    ft.Container(
                        content=ft.Row([
                            ft.Text("语速", size=12, width=80),
        ...
        runtime_tab = ft.Container(
            content=ft.Column(
                [
                    ft.Text("运行参数", size=14, weight=ft.FontWeight.BOLD),
                    ft.Text("配置端口范围和设备模式，确保推理服务稳定运行。", size=11, color=ft.Colors.GREY_600),
                    ft.Divider(height=12),
                    ft.Row([
                    ...
                    ft.Row([
                        ft.Text("设备模式", size=12, width=80),
                        device_mode_dropdown,
                    ], spacing=10),
                    ft.Row([
                        ft.Text("状态刷新(秒)", size=12, width=80),
        ...
        tts_tab = ft.Container(
            content=ft.Column(
                [
                    ft.Text("TTS 接口", size=14, weight=ft.FontWeight.BOLD),
                    ft.Text("选择本地实例或远程 API，适配不同部署方式。", size=11, color=ft.Colors.GREY_600),
                    ft.Divider(height=12),
                    ft.Row([
                        ft.Text("接口模式", size=12, width=80),
                        tts_api_mode_dropdown,
                    ], spacing=10),
                    ft.Row([
                        ft.Text("远程地址", size=12, width=80),
                        tts_remote_base_url_field,
                    ], spacing=10),
                    ft.Container(
                        content=ft.Text(
                            "远程模式将直接请求远端的 /update_prompt_audio 与 /gen_single 接口，适合云端或集群部署。",
                            size=11,
                            color=ft.Colors.GREY_600,
                            italic=True,
                        ),
        ...
        ai_tab = ft.Container(
            content=ft.Column(
                [
                    ft.Text("AI 角色识别", size=14, weight=ft.FontWeight.BOLD),
                    ft.Text("让 AI 帮你识别文本角色并自动匹配合适音色。", size=11, color=ft.Colors.GREY_600),
                    ft.Divider(height=12),
                    ft.Row([ai_enabled_switch], spacing=10),
                    ft.Row([
                        ft.Text("API Key", size=12, width=80),
                        ai_api_key_field,
                    ], spacing=10),
                    ft.Row([
                        ft.Text("接口模式", size=12, width=80),
                        ai_api_url_mode_dropdown,
                    ], spacing=10),
                    ft.Row([
                        ft.Text("基础地址", size=12, width=80),
                        ai_base_url_field,
                    ], spacing=10),
                    ft.Row([
                        ft.Text("自定义 Base", size=12, width=80),
                        ai_custom_base_url_field,
                    ], spacing=10),
                    ft.Row([
                        ft.Text("模型名称", size=12, width=80),
                        ai_model_field,
                    ], spacing=10),
                    ft.Row([
                    ...
                    ft.Row([
                        ft.Text("最大 Token", size=12, width=80),
                        ai_max_tokens_field,
                    ], spacing=10),
                    ft.Row([
                        ft.Text("分段范围", size=12, width=80),
                        ai_seg_min_cn_field,
                        ai_seg_max_cn_field,
                    ], spacing=10),
                    ft.Row([
                        ai_adjust_speed_switch,
                        ai_adjust_emotion_switch,
                    ], spacing=10),
                    ft.Container(
                        content=ft.Text(
                            "提示：本地推理可不填 Key，接入云端服务时需填写对应密钥；\n"
                            "若服务使用 /v4 等非默认路径，请选择“自定义”并在自定义 Base 中写全版本路径。",
                            size=11,
                            color=ft.Colors.GREY_600,
                            italic=True,
                        ),
        ...
        advanced_tab = ft.Container(
            content=ft.Column(
                [
                    ft.Text("高级与诊断", size=14, weight=ft.FontWeight.BOLD),
                    ft.Text("调节日志级别与输出策略，便于排查问题。", size=11, color=ft.Colors.GREY_600),
                    ft.Divider(height=12),
                    ft.Row([
                        ft.Text("日志级别", size=12, width=80),
                        log_level_dropdown,
                    ], spacing=10),
                    ft.Container(
                        content=ft.Text(
                            "DEBUG 会记录最详细的调试信息，ERROR 仅保留错误日志，建议日常使用 INFO。",
                            size=11,
                            color=ft.Colors.GREY_600,
                            italic=True,
                        ),
        ...
        tabs = ft.Tabs(
            ...
            expand=1,
            tabs=[
                ft.Tab(text="外观", icon=ft.Icons.BRUSH, content=appearance_tab),
                ft.Tab(text="运行", icon=ft.Icons.MEMORY, content=runtime_tab),
                ft.Tab(text="TTS", icon=ft.Icons.SURROUND_SOUND, content=tts_tab),
                ft.Tab(text="AI", icon=ft.Icons.SMART_TOY, content=ai_tab),
                ft.Tab(text="高级", icon=ft.Icons.TUNE, content=advanced_tab),
            ],
        )
        ...
        header = ft.Container(
            content=ft.Row(
                [
                    ft.Container(
                        content=ft.Icon(ft.Icons.SETTINGS_SUGGEST, size=26, color=ft.Colors.WHITE),
                        width=44,
                        height=44,
                    ...
                    ft.Column(
                        [
                            ft.Text("应用设置中心", size=16, weight=ft.FontWeight.BOLD),
                            ft.Text("集中管理界面外观、运行参数与智能配置。", size=11, color=ft.Colors.GREY_600),
                        ],
                        spacing=2,
