...
class IndexTTS:
    def __init__(
        ...
        self.bigvgan.load_state_dict(vocoder_dict["generator"], strict=False)
        self.bigvgan.eval()
    ...
    def infer_fast(self, audio_prompt, text, output_path, verbose=False, max_text_tokens_per_sentence=100,
        ...
        if self.cache_cond_mel is None or self.cache_audio_prompt != audio_prompt:
            ...
            cond_mel = MelSpectrogramFeatures(n_mels=1024, n_fft=2048)(audio).to(self.device)
            bigvgan_mel = MelSpectrogramFeatures(n_mels=80, n_fft=1024)(audio).to(self.device)
            ...
            self.cache_bigvgan_mel = bigvgan_mel
        else:
            cond_mel = self.cache_cond_mel
            bigvgan_mel = self.cache_bigvgan_mel
            cond_mel_frame = cond_mel.shape[-1]
            pass

        auto_conditioning = cond_mel.transpose(1, 2)
        ...
        for item_tokens in all_text_tokens:
            ...
            with torch.no_grad():
                with torch.amp.autocast(batch_text_tokens.device.type, enabled=self.dtype is not None,
                                        dtype=self.dtype):
                    temp_codes, _ = self.gpt.inference_speech(auto_conditioning, batch_text_tokens,
        ...
        for batch_codes, batch_tokens, batch_sentences in zip(all_batch_codes, all_text_tokens, all_sentences):
            for i in range(batch_codes.shape[0]):
                ...
                text_tokens = batch_tokens[i].unsqueeze(0)
                all_idxs.append(batch_sentences[i]["idx"])
                m_start_time = time.perf_counter()
                with torch.no_grad():
                    with torch.amp.autocast(text_tokens.device.type, enabled=self.dtype is not None, dtype=self.dtype):
                        cond_len = torch.tensor([auto_conditioning.shape[-1]], device=text_tokens.device)
                        latent = \
                            self.gpt(auto_conditioning, text_tokens,
                                     torch.tensor([text_tokens.shape[-1]], device=text_tokens.device), codes,
                                     torch.tensor([codes.shape[-1]], device=text_tokens.device),
                                     auto_conditioning,
                                     cond_mel_lengths=cond_len,
                                     emo_cond_mel_lengths=cond_len,
                                     use_speed=torch.zeros(1, dtype=torch.long, device=text_tokens.device),
                                     do_spk_cond=True)
        ...
        for sent in sentences:
            ...
            with torch.no_grad():
                ...
                with torch.amp.autocast(text_tokens.device.type, enabled=self.dtype is not None, dtype=self.dtype):
                    ...
                    m_start_time = time.perf_counter()
                    wav, _ = self.bigvgan(latent, bigvgan_mel)
                    bigvgan_time += time.perf_counter() - m_start_time
                    wav = wav.squeeze(1)
